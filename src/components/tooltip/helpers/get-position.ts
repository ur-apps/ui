...
export function getTooltipPosition(
  ...
  for (const fallback of PLACEMENT_FALLBACKS[placement]) {
    if (placementsAvailability[fallback]) {
      return positionFunctions[fallback](targetRect, tooltipRect);
    }
  }
...
function getTopPosition(targetRect: DOMRect, tooltipRect: DOMRect): { top: number; left: number } {
  ...
  return {
    top: Math.max(targetRect.top - tooltipRect.height, 0),
    left: left,
  };
}

function getBottomPosition(targetRect: DOMRect, tooltipRect: DOMRect): { top: number; left: number } {
  const left = calculateConstrainedPosition({
    idealPosition: targetRect.left + targetRect.width / 2 - tooltipRect.width / 2,
    targetBoundMin: targetRect.right - tooltipRect.width,
    targetBoundMax: targetRect.left,
...
function getLeftPosition(targetRect: DOMRect, tooltipRect: DOMRect): { top: number; left: number } {
  ...
  return {
    top: top,
    left: Math.max(targetRect.left - tooltipRect.width, 0),
  };
}

function getRightPosition(targetRect: DOMRect, tooltipRect: DOMRect): { top: number; left: number } {
  const top = calculateConstrainedPosition({
    idealPosition: targetRect.top + targetRect.height / 2 - tooltipRect.height / 2,
    targetBoundMin: targetRect.bottom - tooltipRect.height,
    targetBoundMax: targetRect.top,
